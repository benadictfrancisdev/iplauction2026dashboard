...
export function useAuctionData() {
  ...
  useEffect(() => {
    ...
    // Realtime — only subscribe to specific tables needed
    const channel = supabase
      ...
      .on('postgres_changes', { event: '*', schema: 'public', table: 'auction_log' },
        () => fetchLog())
      .on('postgres_changes', { event: '*', schema: 'public', table: 'retained_players' },
        () => fetchRetained())
      .subscribe(status => setIsLive(status === 'SUBSCRIBED'));

    // Safety fallback — 30s (realtime handles most updates)
